...
// ---------------------------------------------------------------------------
// MOCK DATA — stand-in for the GET /users/:userId/profile response.
// Delete this block once the real API is wired up; the shape here should
...
export default function ProfilePagePreview() {
  ...
  const {
    name, title, avatarUrl, verified, online, location, memberSince,
    rating, reviewCount, responseTime, hourlyRate, bio,
    skills = [], languages = [], stats = {}, portfolio = [],
  } = profile;
            ...
            <CardContent className="flex flex-col gap-6 p-6 sm:flex-row sm:items-start">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="font-serif text-2xl font-semibold text-[#14213D]">{name}</h1>
                  {verified && (
                    <span className="inline-flex items-center gap-1 text-[#C9971D]" title="Identity verified">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-[#5C6370]">
                  ...
                  {typeof rating === 'number' && (
                    <span className="inline-flex items-center gap-1">
                      <Star className="h-4 w-4 fill-[#C9971D] text-[#C9971D]" aria-hidden="true" />
                      <span className="font-mono font-semibold text-[#14213D]">{rating.toFixed(1)}</span>
              ...
              <div className="flex shrink-0 flex-col items-start gap-2 sm:items-end">
                {typeof hourlyRate === 'number' && (
                  <p className="font-mono text-xl font-semibold text-[#14213D]">
                    ...
                    <span className="text-sm font-normal text-[#5C6370]">/hr</span>
                  </p>
                )}
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" className="border-[#14213D] text-[#14213D] hover:bg-[#14213D]/5">
                    Hire
                  </Button>
                </div>
              </div>
          ...
          <div className="space-y-6 lg:col-span-2">
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show" custom={1}>
                <Card className="border-none bg-white shadow-sm">
                  <CardContent className="p-6">
                    <h2 className="mb-3 font-serif text-lg font-semibold text-[#14213D]">About</h2>
                    <p className="whitespace-pre-line text-sm leading-relaxed text-[#5C6370]">{bio}</p>
                  </CardContent>
                </Card>
              </motion.div>
            )}
            ...
            {skills.length > 0 && (
              <motion.div variants={fadeUp} initial="hidden" animate="show" custom={2}>
                <Card className="border-none bg-white shadow-sm">
                  <CardContent className="p-6">
                    <h2 className="mb-3 font-serif text-lg font-semibold text-[#14213D]">Skills</h2>
            ...
            {portfolio.length > 0 && (
              <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3}>
                <Card className="border-none bg-white shadow-sm">
                  <CardContent className="p-6">
                    <h2 className="mb-3 font-serif text-lg font-semibold text-[#14213D]">Portfolio</h2>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                      {portfolio.map((item) => (
                        <div key={item.id} className="group aspect-square overflow-hidden rounded-lg bg-[#14213D]/5">
                          <img
                            src={item.imageUrl}
                            alt={item.title}
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="show" custom={2}>
              <Card className="border-none bg-white shadow-sm">
                <CardContent className="space-y-4 p-6">
                  <h2 className="font-serif text-lg font-semibold text-[#14213D]">At a glance</h2>
                  {typeof stats.jobsCompleted === 'number' && (
                    <StatBlock icon={Briefcase} label="Jobs completed" value={stats.jobsCompleted} />
                  )}
                  {typeof stats.hoursWorked === 'number' && (
                    <StatBlock icon={Clock} label="Hours worked" value={stats.hoursWorked} />
                  )}
                  {responseTime && (
                    <StatBlock icon={ShieldCheck} label="Response time" value={responseTime} />
                  )}
                </CardContent>
              </Card>
            </motion.div>
